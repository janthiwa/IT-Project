const validateAppointment = (data) => {
    ...
    if (!data.doctor_name || data.doctor_name.trim() === "") errors.push('กรุณากรอกชื่อคุณหมอ');
    if (!data.app_date) errors.push('กรุณาเลือกวันที่นัดหมาย');
    if (!data.app_time) errors.push('กรุณาเลือกเวลานัดหมาย');
    if (!data.location || data.location.trim() === "") errors.push('กรุณาระบุสถานที่ (ถ้าเลือกอื่นๆ อย่าลืมพิมพ์ระบุ)');
    if (!data.note || data.note.trim() === "") errors.push('กรุณากรอกหมายเหตุ');
...
const validateRegistration = (data) => {
    ...
    if (!data.firstname || data.firstname.trim() === "") errors.push('กรุณากรอกชื่อผู้ป่วย');
    if (!data.lastname || data.lastname.trim() === "") errors.push('กรุณากรอกนามสกุลผู้ป่วย');
    

    if (!data.id_card || data.id_card.trim().length !== 13) {
    ...
    if (!data.diagnosis || data.diagnosis.trim() === "") errors.push('กรุณากรอกผลการวินิจฉัย');
