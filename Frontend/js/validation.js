// validate การบันทึกผู้ป่วย
const validateRegistration = (data) => {
    let errors = [];
    

    if (!data.firstname || data.firstname.trim() === "") 
        errors.push('กรุณากรอกชื่อผู้ป่วย');


    if (!data.lastname || data.lastname.trim() === "") 
        errors.push('กรุณากรอกนามสกุลผู้ป่วย');
    

    if (!data.id_card || data.id_card.trim().length !== 13) {
        errors.push('กรุณากรอกเลขบัตรประชาชนให้ครบ 13 หลัก');
    }


    if (!data.birthday) {
        errors.push('กรุณาเลือกวันเดือนปีเกิดของผู้ป่วย');
    }


    if (!data.age || data.age <= 0) 
        errors.push('กรุณากรอกอายุผู้ป่วยให้ถูกต้อง');


    if (!data.checkup_date)
        errors.push('กรุณาระบุวันที่เข้าตรวจ');


    if (!data.gender)
        errors.push('กรุณาเลือกเพศผู้ป่วย');
    

    if (!data.congenital_disease || data.congenital_disease.trim() === "") {
        errors.push('กรุณาเลือกโรคประจำตัวอย่างน้อย 1 รายการ');
    }


    if (!data.diagnosis || data.diagnosis.trim() === "") errors.push('กรุณากรอกผลการวินิจฉัย (หากไม่มีให้พิมพ์ - )');

    return errors;
};





// validate การออกใบนัด
const validateAppointment = (data) => {
    let errors = [];


    if (!data.user_id || data.user_id === "" || data.user_id.includes("กรุณาเลือก")) {
        errors.push('กรุณาเลือกคนไข้');
    }


    if (!data.doctor_name || data.doctor_name.trim() === "") {
        errors.push('กรุณากรอกชื่อคุณหมอ');
    }


    if (!data.app_date) {
        errors.push('กรุณาเลือกวันที่นัดหมาย');
    }


    if (!data.app_time) {
        errors.push('กรุณาเลือกเวลานัดหมาย');
    }


    if (!data.location || data.location.trim() === "" || data.location.includes("กรุณาเลือก")) {
        errors.push('กรุณาระบุสถานที่นัดหมาย');
    }


    if (!data.note || data.note.trim() === "") {
        errors.push('กรุณากรอกหมายเหตุ (หากไม่มีให้พิมพ์ - )');
    }

    return errors;
};
